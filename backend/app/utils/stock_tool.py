...
class StockTool:
    ...
    def _to_eastmoney_code(self, symbol: str) -> str:
        ...
        """
        # 特殊处理指数
        if symbol == '000001' and not symbol.startswith('6'):
            # 这里逻辑需要小心，000001 既是平安银行也是上证指数
            # 在 A 股习惯中，'000001' 作为 6 位纯数字通常指股票
            # 但在指数场景下，上证指数代码是 000001
            # 这里的 symbol 建议传入时带前缀或区分逻辑
            # 根据本项目习惯，股票代码 000001 是深圳，1.000001 是上证指数
            pass
        ...
        if symbol.lower() in index_map:
            return index_map[symbol.lower()]
