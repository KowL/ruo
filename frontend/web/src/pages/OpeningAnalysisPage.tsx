...
import { runOpeningAnalysis, getAnalysisReport } from '@/api/analysis';
...
const OpeningAnalysisPage: React.FC = () => {
    ...
    React.useEffect(() => {
        const fetchInitialReport = async () => {
            ...
            try {
                // 1. 优先尝试从数据库直接 GET 报告
                const res = await getAnalysisReport('opening_analysis', selectedDate);
                if (res && res.success && res.result) {
                    ...
                    setResult(null);
                    setReportContent('');
                    setError(res?.message || '该日期暂无开盘分析报告，请点击分析开始');
                }
            } catch (err: any) {
                setResult(null);
                setReportContent('');
    ...
    const handleAnalysis = async (forceRerun: boolean = false) => {
        ...
        try {
            // 调用运行分析的 POST 接口
            const res = await runOpeningAnalysis(selectedDate, forceRerun);
            if (res && res.success) {
                if (res.message && res.message.includes('请等待')) {
                    setIsPolling(true);
                } else if (res.result) {
                    setResult(res.result.data || res.result);
                    setReportContent(res.result.markdown || '');
    ...
    return (
        <div className="p-6 space-y-6 animate-fade-in pb-24 lg:pb-6">
            <div className="flex flex-col space-y-1 mb-2">
                <h1 className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-white to-gray-400">开盘分析</h1>
                <p className="text-sm text-[var(--color-text-secondary)]">基于昨日涨停与今日竞价数据的早盘策略分析</p>
            </div>
...
export default OpeningAnalysisPage;
